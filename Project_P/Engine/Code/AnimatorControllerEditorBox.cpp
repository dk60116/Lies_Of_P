...
void CAnimatorControllerEditorBox::RenderGraph()
{
    ...
    const bool graphHovered = ImGui::IsWindowHovered(ImGuiHoveredFlags_AllowWhenBlockedByActiveItem);

    if (graphHovered && ImGui::IsMouseDragging(ImGuiMouseButton_Middle))
    {
    ...
    if (graphHovered && ImGui::IsMouseClicked(ImGuiMouseButton_Right))
    {
        m_pendingTransitionFrom.clear();
        m_pendingSourceType = EPendingSource::None;
    }
    ...
    for (_int i = 0; i < (_int)m_transitions.size(); ++i)
    {
        ...
        if (tr.isAny)
        {
            ...
            if (graphHovered && DistancePointToSegment(mousePos, from, to) <= 6.f)
            {
